...
window.addEventListener("DOMContentLoaded",
    function(){
        if(typeof localStorage === "undefined"){
            window.alert("このブラウザは機能が実...されていません")
            return;
        }else{
            viewStorage();
            saveLocalStorage();
    ...
    save.addEventListener("click",
        function (e){
            ...
            if(key === "" || value === ""){
                ...
                let w_confirm = window.confirm("LocalStorageにキーメモを保存しますか？");
                if(w_confirm === true){
    ...
    del.addEventListener("click",
        function(e) {
            ...
            if(w_sel === "1"){
                const key = document.getElementById("textKey").value;
                const value = document.getElementById("textMemo").value;
                let w_confirm = window.confirm("LocalStorageからキーメモを削除しますか？");
                if(w_confirm === true){
